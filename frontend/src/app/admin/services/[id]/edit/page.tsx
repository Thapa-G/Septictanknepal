...
export default function AdminEditServicePage({
  ...
  useEffect(() => {
    async function loadService() {
      try {
        const service = await servicesService.getBySlug(serviceId);
        setFormData({
          title: service.title || '',
          heading: service.heading || '',
          slug: service.slug || '',
          category: service.category || 'Plumbing & Drainage',
          icon: service.icon || 'plumbing',
          ...
          og_image_alt: service.og_image_alt || '',
        });

        if (service.faqs && service.faqs.length > 0) {
          setFaqs(service.faqs.map((f) => ({ question: f.question, answer: f.answer })));
        } else {
  ...
  if (loading) {
    return (
      <AdminLayoutShell title="Edit Service">
  ...
  return (
    <AdminLayoutShell
      ...
      subtitle="Update service specifications, media, and FAQs"
      actions={
        <Link
          href="/admin/services"
          ...
          <div className="flex justify-between items-center border-b border-[#cbd5e1] pb-3">
            <div>
              ...
              <p className="text-[13px] text-[#475569]">
                Update the accordion Q&amp;A items for this service.
              </p>
            </div>
            <button
              type="button"
